...
class TestMinMaxSum(unittest.TestCase):
    ...
    def test_min_is_32bit_max_is_64bit(self):
        ...
        self.assertEquals(
            min_max_sum([1, 2, 3, 4, TestMinMaxSum.INT32_MAX_VALUE]), (10, expected_max)
        )

    def test_both_sums_are_64bit(self):
        expected_min = 1 + 2 + 3 + TestMinMaxSum.INT32_MAX_VALUE
        expected_max = (
            2 + 3 + TestMinMaxSum.INT32_MAX_VALUE + (TestMinMaxSum.INT32_MAX_VALUE + 1)
        )
        self.assertEquals(
            min_max_sum(
                [
